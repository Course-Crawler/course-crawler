...
fn main() {
    ...
    let profiles = opt.profiles.split(",").collect::<Vec<&str>>();

    // client profile
    if profiles.contains(&"client") || profiles.contains(&"*") {
        ...
        println!("Launching course crawler with {} recorder replicas...", opt.replicas.to_string().blue());
    ...
    // server profile
    if profiles.contains(&"server") || profiles.contains(&"*") {
        println!("Launching course crawler video server...");
    }
    ...
    let status = Command::new("sh")
        .arg("-c")
        .arg(format!(
            "COMPOSE_PROFILES={} docker compose down && COMPOSE_PROFILES={} docker compose up --build -d",
            profiles.join(","),
            profiles.join(",")
        ))
        .status()
